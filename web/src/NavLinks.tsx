...
/** 객체 이동 링크 묶음 — 카드·패널·토스트·배너 어디서나 같은 글자, 같은 순서(3D · 모니터링 · 카드), 같은 자리(줄의 오른쪽 끝).
...
export default function NavLinks({ modelId, gid, viewer, onViewer, style }: { modelId: string; gid?: string | null; viewer?: string; onViewer?: () => void; style?: CSSProperties }) {
  ...
  const card = links && here !== 'fm' ? <a href={links.fm} onClick={stop} title="시설관리에서 이 자산의 카드" style={a}>카드</a> : null
