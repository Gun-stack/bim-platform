...
export default function ObjectDrawer({ modelId, gid, tick, reload, onClose }: { modelId: string; gid: string; tick?: unknown; reload?: () => Promise<unknown>; onClose: () => void }) {
  ...
  return (
    <aside style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 440, background: '#fff', boxShadow: '-4px 0 20px #0002', padding: 14, overflow: 'auto', fontSize: 13, zIndex: 40, boxSizing: 'border-box', fontFamily: 'system-ui' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
        <b>객체</b><span style={{ color: '#999', fontSize: 11, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 }} title={gid}>{gid}</span>
        <a href={links.viewer} title="뷰어에서 구역 강조 + 비콘" style={btn}><ExternalLink size={12} /> 3D 위치</a>
        <X size={16} style={{ cursor: 'pointer', color: '#666' }} onClick={onClose} />
      </div>
      {!d ? <p style={{ color: '#999' }}>불러오는 중…</p> : <>
