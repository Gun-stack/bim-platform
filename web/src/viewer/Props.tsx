        ...
        <details key={pset} open={pset.startsWith('Pset_')}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}><tbody>
            {Object.entries(props).map(([k, v]) => (
              <tr key={k} style={{ borderTop: `1px solid ${T.bg.line}` }}><td style={{ color: T.ink[2], padding: '2px 4px', whiteSpace: 'nowrap' }}>{k}</td><td style={{ padding: '2px 4px', wordBreak: 'break-all' }}>{String(v)}</td></tr>
            ))}
          </tbody></table>
        </details>
      ))}
      {e.properties.Pset_BimStatus && <details><summary style={{ color: T.ink[2] }}>Pset_BimStatus 원본</summary><pre style={{ fontSize: 11, color: T.ink[2], whiteSpace: 'pre-wrap', margin: '4px 0' }}>{JSON.stringify(e.properties.Pset_BimStatus, null, 1)}</pre></details>}
    </>
  )
}
    ...
    <>
      ...
      {details.length > 0 && <>
        ...
        {common.length ? <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}><tbody>
          {common.map(([k, v]) => <tr key={k} style={{ borderTop: `1px solid ${T.bg.line}` }}><td title={k} style={{ width: '50%', color: T.ink[2], padding: '2px 4px', fontSize: 12, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{k}</td><td style={{ padding: '2px 4px', overflowWrap: 'anywhere' }}>{v}</td></tr>)}
        </tbody></table> : <div style={{ color: T.ink[2] }}>모두 같은 값인 속성 없음</div>}
      </>}
    </>
  )
}
