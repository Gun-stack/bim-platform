...
export const SYSTEM_COLOR: Record<string, number> = { ELECTRICAL: 0xf59e0b, DOMESTICCOLDWATER: 0x2563eb, WASTEWATER: 0x78350f, FIREPROTECTION: 0xdc2626, SIGNAL: 0x9333ea, AIRCONDITIONING: 0x0d9488, CHILLEDWATER: 0x0284c7, VENTILATION: 0x65a30d, DOMESTICHOTWATER: 0xe11d48, GAS: 0xca8a04, DATA: 0x4f46e5, LIGHTING: 0xeab308,
  ...
  return (
    <div style={{ padding: '6px 10px', background: abnormal.length ? '#fff5f5' : '#f0fdf4', borderBottom: '1px solid #e5e5e5' }}>
      <div onClick={() => setCollapsed(!collapsed)} style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
        ...
        <span style={{ fontSize: 12, fontWeight: 600, color: abnormal.length ? '#dc2626' : '#16a34a' }}>{abnormal.length ? `이상 ${abnormal.length}` : '전부 정상'}</span>
        {power?.source === 'GENERATOR' && <span style={{ fontSize: 11, color: '#9a3412' }}>· 정전</span>}
        {collapsed ? <ChevronDown size={14} style={{ color: '#888' }} /> : <ChevronUp size={14} style={{ color: '#888' }} />}
      </div>
      {!collapsed && <>
      <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#444', margin: '6px 0 4px' }}>
