...
**2. 3D 뷰어 — 고르면 "무엇·어디·누가"가 한 카드에.** 요소를 클릭하면 우측 상단 카드에 이름·상태·종류/위치·소속 계통(팀 색)·자산 태그·열린 작업지시가 모이고, 그 아래 **운영 상태**에서 경보/장애/복구와 계측값(부하·온도 등)을 바로 고칠 수 있습니다. 좌측 최상단 **상태판**은 어느 탭에서든 이상 목록을 보여줍니다.
...
**6. 설비 모니터링 — 요약 → 처리할 것 → 팀별 핵심 장비 → 층×팀 격자.** 첫 줄은 건물 총계(경보·장애·계측 주의·열린 작업지시·전원·주차 점유). 그 아래 **지금 처리할 것**이 우선순위순으로 나오고, 팀 카드를 고르면 그 팀의 **핵심 장비 카드**(수신기·소화펌프·변압기·냉동기…)가 계측값과 함께 뜹니다. 층×팀 격자는 이상만/장비/전체로 걸러 보고, 새 경보는 4초간 강조되고 알림음을 켤 수 있습니다. 우측(또는 아래) **최근 이벤트**가 "언제 무슨 일이"를 답합니다.
...
**8. 가상 건물.** 위 화면의 건물은 IfcOpenShell API로 직접 생성한 36×16 m 업무동입니다 — 실무 관행대로 B2 변전·발전기·기계·수조실, B1 주차장·방재·통신·주차관제실, 옥탑 보일러실, 계단실 2개소, 17% 차량 램프, 슬래브 개구부. 14계통 474요소 521연결에 운영 상태가 들어 있어 위 시나리오를 바로 돌려볼 수 있습니다.
