...
**2. 3D 뷰어 — 고르면 "무엇·어디·누가"가 한 카드에.** 요소를 클릭하면 우측 상단 카드에 이름·상태·종류/위치·소속 계통(분야 색)·자산 태그·열린 작업지시가 모이고, 그 아래 **운영 상태**에서 경보/장애/복구와 계측값(부하·온도 등)을 바로 고칠 수 있습니다. 좌측 최상단 **상태판**은 어느 탭에서든 이상 목록을 보여줍니다. 고른 객체는 URL(`?sel=`)과 좌하단 **맥락 독**에 남아 모니터링·시설관리로 넘어가도 따라갑니다.

![요소 선택 — 요약 카드와 운영 상태](images/02-viewer-selected.png)

**3. 설비 계통 — 색으로 보고, 흐름을 따라간다.** 계통 탭은 분야(소방·수송·설비·통신제어·전기)별로 묶여 있고 "계통별 색으로 보기"를 켜면 건물 안의 배관·트레이·장비가 계통 색으로 칠해집니다. 장비를 고르고 **하류(말단까지)/상류(원천까지)** 를 누르면 재귀 CTE가 그래프를 따라가 3D에 경로를 칠하고, 상단 배너에 "몇 요소 · 어느 층 · 주요 종류"를 요약합니다.
...
**6. 설비 모니터링 — 요약 → 조치 필요 → 분야별 핵심 장비 → 층×분야 격자.** 첫 줄은 건물 총계(경보·장애·계측 주의·열린 작업지시·전원·주차 점유). 그 아래 **조치 필요**가 우선순위순으로 나오고, 분야 카드를 고르면 그 분야의 **핵심 장비 카드**(수신기·소화펌프·변압기·냉동기…)가 계측값과 함께 뜹니다. 층×팀 격자는 이상만/장비/전체로 걸러 보고, 새 경보는 4초간 강조되고 알림음을 켤 수 있습니다. 우측(또는 아래) **최근 이벤트**는 op_event 이력이라 지나간 일이 사라지지 않고, 계측값이 있는 장비는 📈 아이콘으로 **계측 트렌드**(시계열 차트)를 열 수 있습니다. 점검 주기를 넘긴 자산은 총계·분야 카드·목록에 **점검 지연**으로 함께 올라옵니다. 맨 아래 **경보 통계**는 같은 이력에서 분야별 발생 건수·평균 복구 시간·미복구·재발 장비(7·30·90일)를 냅니다 — 격자가 "지금"이라면 통계는 "얼마나 자주, 얼마나 오래".

![설비 모니터링 — 소방 선택](images/08-monitor-team.png)
...
- **현장 중심의 계통 해석** — 전기·소방·설비·통신제어·수송 분야 기준 상태판과 원천 → 말단 그래프를 결합해 경보 위치뿐 아니라 원인과 영향 범위를 함께 보여줍니다.
