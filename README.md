...
- FMS·MEP 실무 경험을 바탕으로 업무 흐름과 기술 구현을 보여주는 **포트폴리오용 개인 프로젝트**. 실제 운영·상용 서비스 아님
- 제품 기획·도메인 모델링부터 백엔드·프론트엔드·IFC 변환 워커·DB·Docker 실행 환경까지 엔드투엔드 설계·구현
- AI 페어 프로그래밍으로 단기 집중 개발. 설계 결정·도메인 규칙·검증은 직접 판단

![데모: 모니터링 → 경보 포커스 → 계통 추적 → 정전 시나리오 → 작업지시 보드](images/demo.gif)

> 📐 화면 구조·기능 배치·화면 간 연관 관계·용어 사전: [화면 설계서](docs/screen-design.md). 세 화면의 역할은 **뷰어 = 지역, 모니터링 = 현재, 시설관리 = 할 일**
...
- 요소 클릭 → 우측 카드에 이름·상태·종류/위치·소속 계통(분야 색)·자산 태그·열린 작업지시
- 카드 아래 **운영 상태**: 경보/장애/복구, 계측값(차단기·부하 등) 즉시 수정
- 좌측 최상단 **상태판**: 어느 탭에서든 이상 목록
- 선택은 URL(`?sel=`)과 **맥락 독**(3D 썸네일 카드)에 남아 모니터링·시설관리까지 동행
- 키보드만으로 조작: `W/A/S/D` `Q/E` 비행(`Shift` 4배속), 방향키 궤도 회전, `+/-` 줌, `Home`/`F`/`1`/`3`/`7` 뷰, `Tab`/`⇧Tab` 요소 순환, `I`/`H`/`⇧H`/`⌥H` 격리·숨김, `B`/`O`/`Z`/`R` 구조체·개구부·공간·병합, `C`/`M`/`N`/`G`/`P`/`T`/`K` 도구·색상, `[`/`]` 상류·하류 추적, `L` 링크, `?` 안내

![요소 선택 — 요약 카드와 운영 상태](images/02-viewer-selected.png)

**3. 설비 계통 — 색으로 판단, 흐름을 추적**

- 계통 탭은 분야(소방·수송·설비·통신·제어·전기)별 묶음. "계통별 색으로 보기" → 배관·트레이·장비가 계통 색
- 장비 선택 후 **하류(말단까지)/상류(원천까지)** → 재귀 CTE가 그래프를 따라가 3D에 경로 색칠
- 상단 배너에 "몇 요소 · 어느 층 · 주요 종류" 요약. 아래는 저압 배전반 하류 — B2~옥탑 조명·분전반 572요소

![계통 추적 — 저압 배전반의 하류](images/03-route.gif)

**4. 정전 시나리오 — 계통 그래프만으로 영향 범위 계산**

- "정전 시나리오" → ATS 발전기 절체. **비상분전반 하류만 초록(전원 있음), 나머지 짙은 회색(무전원)**
- 죽는 조명·분전반과 살아있는 소화펌프·수신기·비상조명이 한눈에
- 밸브 잠금·차단기 트립도 같은 계산(하류 차집합)

![정전 시나리오 — 전원 있음/없음](images/04-blackout.gif)

**5. 경보 포커스 — 찾아가는 사람의 화면**

- 모니터링·상태판에서 경보 클릭 → 건물 전체 뷰 유지, **해당 구역만 파랗게 강조 + 지붕 위 비콘**
- 줌인·단면보다 "어느 층 어느 구역인가"가 먼저라는 현장 관점

![경보 포커스 — 2F 회의실 연기감지기](images/05-focus.png)

**6. 설비 모니터링 — 총계 → 분야 → 조치 필요 → 핵심 요소 → 층×분야 격자**

- 첫 줄 건물 총계(경보·장애·계측 주의·작업지시·미배정·점검 지연). 숫자 클릭 → 그것들이 보이는 곳으로 이동
- **조치 필요**: 경보 → 장애 → 계측 위험 → 주의 → 점검 지연 순. 이상 행의 **확인** 버튼이 확인 시각 기록 + 강조 완화. 자동 복구 없음 — 해제는 BMS·현장 확인의 몫
- 분야 카드 선택 → 그 분야 **핵심 요소 카드**(수신기·소화펌프·변압기·냉동기…) + 계측값
- 층×분야 격자: 이상만/요소/전체 필터. 층 라벨의 **단면** 링크 → 뷰어를 그 층 높이로 절단
- 새 경보 4초 강조 + 알림음 옵션. **최근 이벤트**는 op_event 이력 — 지나간 일이 사라지지 않음
- **경보 통계**(맨 아래): 분야별 발생 건수·평균 복구 시간·미복구·재발 요소(7·30·90일). 격자가 "지금", 통계가 "얼마나 자주·얼마나 오래"
- 분야·층·모드·기간 필터 전부 해시 쿼리 — "소방 이상만" 화면을 그대로 공유

![설비 모니터링 — 소방 선택](images/06-monitor-team.png)

- 계측값 있는 요소: 행·카드의 **트렌드** 링크, 뷰어 운영 상태의 📈 → **계측 트렌드**(op_event 시계열)

![계측 트렌드 모달 — 소화수조 수위](images/07-trend.png)

- 헤더 **키오스크** 버튼(`?kiosk=1`): 내비 숨김·글자 확대·관제실 TV용 벽면 모드. 현재 필터 유지한 채 진입·해제

![키오스크 모드](images/08-kiosk.png)

**6-1. 화면을 옮겨도 요소는 그대로**

- 모니터링 행 클릭 → 뷰어로 튀지 않고 같은 화면 우측 **객체 패널**(요약·운영 상태·자산/점검/작업지시)
- 세 화면 내비 링크가 `?sel=` 동행. 좌하단 **맥락 독**이 뷰어의 마지막 3D 장면 썸네일 + 3D/시설관리 링크 + 최근 요소 칩. 드래그로 이동

![객체 패널과 맥락 독 — 모니터링에서](images/09-dock.png)

**7. 시설관리 — 경보가 작업지시가 되어 보드에 쌓인다**

- 감지기 경보·장비 장애가 자산이면 **작업지시 자동 생성**(상위 장비 억제·열린 건 재사용·10분 재발 재오픈)
- 대기/진행/완료 칸반. 드래그 즉시 반영 + 되돌리기. 카드 3D 링크 → 저장된 뷰포인트로 뷰어
- 완료했는데 장비가 아직 경보면 토스트에서 **정상 복구** 원클릭. 쌓이기만 하는 열은 접기

![작업지시 보드 — 드래그와 되돌리기](images/10-fm-board.gif)

- **자산 대장**: IFC 요소에서 일괄 등록한 자산(태그 = 종류-층-순번)을 분류·층 필터. 모델에 없는 장비(추가 설치분)는 태그만으로 추가
- 점검 주기(개월) → 다음 점검일 계산. **지연**·**예정 30일** 필터로 예방정비 계획. 지연 자산 행에서 작업지시 즉시 생성
- COBie·BCF 내보내기: 각각 자산 대장·작업지시 보드 헤더

![자산 대장](images/11-fm-assets.png)

**8. 실무 IFC — 업로드만으로**

- Duplex Plumbing(buildingSMART, Revit, IFC2x3): 포트 연결(`IfcRelConnectsPorts`) 485건 → 방향 그래프. IfcSystem 없어도 Pset(System Classification)에서 **계통 7개 유도**. 자산 일괄 등록(장비 40대) 버튼 하나. 아래는 업로드 직후 모니터링
- Clinic HVAC(27MB, 포트 7,390·연결 3,695): 요소 3,704개가 전부 IfcFlowTerminal 같은 IFC2x3 일반 클래스 → 타입(IfcAirTerminalType 등)으로 구체 클래스 복원. 디퓨저 440·VAV 115·팬 8·냉동기 1 장비 인식, 덕트·피팅 3,100개 제외
- Clinic Electrical: 포트 연결 0건(IFC2x3 Coordination View는 전기 회로 미출력) → 조명·콘센트 분류까지만, 추적 불가. 입력 데이터 한계

![실무 IFC — Duplex Plumbing 모니터링](images/12-plumbing-monitor.png)

**9. 가상 건물**

- 위 화면의 건물은 IfcOpenShell API로 생성한 36×16 m 업무동(지상 10층·지하 2층·옥탑) + 주차타워(3층)
- 실무 관행 반영: B2 변전·발전기·기계·수조실, B1 주차장·방재·통신·주차관제실, 옥탑 보일러실, 계단실 2개소, 17% 차량 램프, 슬래브 개구부
- 14계통 1,613요소 2,005연결 + 운영 상태 초기값 → 위 시나리오 즉시 재현
- `gen_mep.py --floors/--annex/--density`: 층수·부속동(주차타워·후생동)·말단 밀도. 대형 인자(20층·부속동 2·격자 2.5 m)는 규모 측정·3D Tiles 기준선
...
- **IFC 변환 파이프라인** — Python·IfcOpenShell 워커가 IFC → glTF. 요소·Pset·공간 계층·설비 계통·연결·지리참조를 PostgreSQL/PostGIS·MinIO 적재. IFC2x3 일반 클래스는 타입으로 복원, IfcSystem 없으면 Pset에서 계통 유도
...
| 서버에서 IFC → GLB 변환 | 브라우저 파싱 부담 감소. 결과물 캐시·배포 용이 |
| PostgreSQL 잡 큐 | 소수 워커·분 단위 작업에 맞춘 운영 구성요소 최소화 |
| 요소 간 방향 그래프 | 원천 → 말단 추적, 재귀 CTE 기반 영향 범위 계산 |
...
- **긴 운영 수명주기** — 준공 IFC를 수정하지 않고 그 위에 자산·점검·작업지시·운영 상태를 적층
- **표준 확장** — COBie 자산·작업 개념, BCF topic·viewpoint 주요 필드 대응. 내보내기 제공, 정식 xlsx·BCF 왕복으로 확장 가능한 구조
- **현장 중심 계통 해석** — 전기·소방·설비·통신·제어·수송 분야 상태판 + 원천 → 말단 그래프. 경보 위치뿐 아니라 원인과 영향 범위까지

## 실무 BIM 툴체인에서의 위치

| 단계 | 현장 도구 | 이 플랫폼과의 관계 |
|---|---|---|
| 저작 | Revit · ArchiCAD · Tekla | IFC 내보내기 결과를 입력(IFC2x3 Coordination View, IFC4 Reference View). 저작 원본은 건드리지 않음 |
| 조정·검토 | Navisworks · Solibri · BIMcollab | 간섭 검토·이슈(BCF)는 이쪽 몫. 작업지시를 BCF 2.1(topic·viewpoint zip)로 내보내 조정 도구로 반환 가능 |
| 공통 데이터 환경(CDE) | Autodesk Construction Cloud(BIM 360) · Trimble Connect | 준공 IFC의 출처. CDE에서 받은 파일을 변환·저장(MinIO)하고 운영 데이터를 덧붙임 |
| 인수인계 | COBie 스프레드시트 | Component/Type/Job 개념을 자산·점검·작업지시로 축소 반영. COBie 시트(CSV zip) 내보내기. 정식 xlsx 는 다음 단계 |
| **운영** | **BMS · 화재수신기 · 주차관제 · FMS** | **이 플랫폼의 자리.** BMS/수신기 값은 상태 API(`PATCH …/status`)로 유입. 경보 → 확인 → 작업지시 → 현장 위치까지 연결 |
...
- 대상: 공개 샘플 중 가장 큰 것들 + 생성기 대형 인자. 어디가 먼저 막히는지 측정
- 환경: 로컬 Docker, MacBook Pro M4 Pro, 헤드리스 Chrome. 로드 시간·draw calls 만 비교값, fps 제외
...
- 공개 샘플 다운로드·생성 인자: [`samples/README.md`](samples/README.md)
- 호스트에 IfcOpenShell 없음 → 생성은 워커 컨테이너에서
...
- `#/map` — GIS 지도
- API 문서(springdoc): [http://localhost:8080/swagger-ui.html](http://localhost:8080/swagger-ui.html) · `GET /v3/api-docs`. 응답이 JSON 맵이라 스키마는 비어 있고, 응답 타입은 [`web/src/api.ts`](web/src/api.ts)가 계약
</details>
