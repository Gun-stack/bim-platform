...
모니터·상태판에서 경보/장애를 클릭하면 뷰어가 `?sel=&focus=1` 로 열리며: 요소가 속한 `IfcSpace` 와 요소만 남기고 나머지 반투명(격리), 그 층 위를 수평 단면으로 잘라내고(`storey.elevation + 3.3`), 구역은 진한 파랑 반투명(`FOCUS_SPACE`), 구역+요소 바운딩에 카메라 핏, 상단 배너 "1F · 1F-B 구역 · 열감지기 4 · 경보". 공간 구조(요소 → Space → Storey)가 DB 에 있어서 좌표를 계산할 필요가 없다 — `spatial_node` 체인만 따라가면 된다.

## 살아 움직이게 — BMS 시뮬레이터

`samples/gen/bms_sim.py <modelId>` 가 3초마다 상태 API 를 친다: 수조 수위·분전반 부하 드리프트, 확률적으로 감지기 경보(몇 틱 뒤 자동 복구)·장애, 펌프 운전/대기 전환, 드물게 정전→복전. 모니터(5초 폴링)와 뷰어 상태판이 따라 움직인다. **실제 BMS/화재수신기 연동도 같은 PATCH 를 치면 된다** — 시뮬레이터는 연동 어댑터의 자리표시자. 자산으로 등록된 감지기가 경보를 내면 작업지시가 자동 생성된다. **중복 억제**: 같은 자산에 열린(OPEN/IN_PROGRESS) 작업지시가 있으면 새로 만들지 않고 재사용하고(응답 `existing: true`), 완료된 뒤 재발하면 새로 만든다 — 반복 경보가 보드를 도배하지 않는 최소 규칙. 세 규칙이 순서대로 걸린다:
1. **상위 장비 억제** — 같은 계통 상류(재귀 CTE, 10단계)에 이미 ALARM/FAULT/OFFLINE 인 장비가 있으면 원인은 그쪽. 하위 요소는 상태만 기록하고 작업지시는 안 만든다(`suppressedBy`). 분전반이 트립되면 조명 12개가 한꺼번에 장애를 내도 작업지시는 분전반 1건.
2. **열린 작업지시 재사용** — 같은 자산에 OPEN/IN_PROGRESS 가 있으면 그것(`existing`).
3. **시간창** — 같은 원인(제목 접두)으로 10분 내 DONE 된 작업지시가 있으면 새로 만들지 않고 다시 연다(`reopened`, `work_order.updated_at` 기준). 플래핑 감지기가 완료 직후 또 울려도 1건.
