...
| M | 동작 목표 (완료 기준) | 학습 노트 |
|---|---|---|
| **M0** 골격 | `docker compose up` 으로 postgis·minio·api(헬스체크)·web(빈 페이지)·worker(idle 루프) 기동. Flyway 초기 스키마. 샘플 IFC 3종 `samples/` 에 배치 | `ifc-basics.md` — STEP/EXPRESS, IFC 파일 구조, 스키마 버전 |
