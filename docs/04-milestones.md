...
- 변환 잡 lease/heartbeat + 모델당 활성 잡 1개(V5), 재변환 정합성, 통합 테스트(api Testcontainers 4 + worker pytest)
- React lazy 코드 분할, Status/FM 애플리케이션 서비스 분리(`FmService`·`StatusService`)
- 작업지시 보드(지라형 칸반, 우선순위·설명 V4)
- 규모 측정·보안 보강 → [05-scale-and-security](05-scale-and-security.md)
