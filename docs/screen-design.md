...
flowchart TB
  ...
  F["시설관리<br/>…/fm"]
  M -- "행 3D 아이콘·핵심 장비 카드 → 구역 강조 ?sel&focus" --> V
  M -. "행 클릭 → 같은 화면 객체 패널 ?sel" .-> M
...
위에서 아래로 **총계 → 팀 → 조치 필요 → 핵심 장비 → 전체 격자**. 요약이 먼저, 상세가 나중. 우측 이벤트 열만 시간축이다.

```text
┌─────────────────────────────────────────────────────┬──────────────┐
│ 건물 총계 — 경보·장애·계측 주의·작업지시·전원·주차     │              │
├─────────────────────────────────────────────────────┤ 최근 이벤트   │
│ 분야 현황 — 카드=분야 필터, 점검 지연 건수 NEW         │ (op_event    │
├─────────────────────────────────────────────────────┤  시간순) NEW │
│ 조치 필요 — 경보→장애→계측 위험→주의→지연 NEW    │              │
├─────────────────────────────────────────────────────┤ 클릭 →       │
│ 핵심 장비 카드 (팀 선택 시) — 계측값 + 📈 트렌드 NEW  │ 뷰어/칸반    │
├─────────────────────────────────────────────────────┤              │
│ 층 × 분야 격자 — 이상만/장비/전체, 행 📈 NEW          │              │
└─────────────────────────────────────────────────────┴──────────────┘
```

- **최근 이벤트 (NEW)** — `op_event` 이력을 읽는다. 이전에는 현재값에서 유도해 다음 변경 때 사라졌다.
- **계측 트렌드 모달 (NEW)** — 숫자 계측값이 있는 행·카드에서 진입(모니터링은 hover `트렌드` 텍스트 링크, 뷰어 운영 상태 헤더는 📈 아이콘). 키별 미니 라인차트(hover 크로스헤어), 값·시각은 텍스트로, warn/crit일 때만 경고색.
- **점검 지연 노출 (NEW)** — 총계 줄·팀 카드에 건수, "조치 필요" 끝(긴급도 최하위)과 행 뱃지. ACTIVE 자산만 센다.
- **키오스크 (NEW)** — 이벤트 열 300→400px. 벽면에서는 "최근 무슨 일"이 격자보다 자주 읽힌다.
- 기존: 새 경보 4초 플래시 + 알림음, `?sel=` 행 스크롤·플래시, 층 라벨 클릭 = 층 필터, 층 단면 링크(상한은 같은 동의 다음 층 표고 — 모니터 API 의 building 열 기준).
...
| 경보 확인 ACK (NEW 09-03) | 모니터링 이상 행의 `확인` 버튼 → `✓ 시각` 표시 + 강조 완화 | "누가 봤는지"가 관제 흐름의 빠진 마디였다. 인증이 없으므로 확인자 대신 확인 시각만 | 기존 상태 PATCH 재사용 — `Pset_BimStatus.AckAt`(ISO), 상태 전이 패치가 자동으로 지움(`statusPatchFor`). op_event 에 이력 자동 축적 |
| WO 완료 → 복구 제안 (NEW 09-03) | 칸반에서 완료 시 장비가 아직 경보·장애면 토스트에 `정상 복구` 원클릭 | 완료된 WO 옆에 경보가 계속 살아 있던 단절. 자동 복구는 안 한다 — 실제 해제는 BMS/현장 확인의 몫 | `GET /status` 1회 조회 → `patchStatus(NORMAL)` |
| 점검 예정 뷰 (NEW 09-03) | 자산 대장 `예정 30일 N` 필터(날짜순 정렬) + 지연 자산 행 `작업지시` 원클릭(기한 7일) | 지연(지난 것)만 보이고 예정(올 것)이 없어 예방정비 계획이 안 됐다 | 프런트 필터만 — `nextDueOn` 이 이미 응답에 있음. WO 생성은 기존 `POST /assets/{id}/work-orders` |
| 필터 딥링크 (NEW 09-03) | 모니터링 팀·층·모드·기간, 칸반 검색·팀·담당·초과 → 해시 쿼리 | §7-3 자기 원칙 준수 — "소방 이상만" 화면을 공유·북마크 | `useHashQuery.setHashParam` (replaceState — hashchange 루프 없음) |
| 키오스크 진입·이탈 (NEW 09-03) | 모니터링 헤더 `키오스크` 버튼 ↔ 벽면 모드의 `키오스크 해제` | URL 수기 입력 없이는 못 들어가던 모드. 현재 필터 유지한 채 전환 | `?kiosk=1` (판정은 URLSearchParams — includes 오탐 제거) |
...
11. **시각 표기는 `ui.ts` 포맷터만** — `day`(YYYY-MM-DD)·`hm`·`hms`·`dateTime`(ko-KR·24시 고정). `toLocale*()` 직접 호출 금지 — 브라우저 로케일에 따라 12시제로 갈린다. "오늘"은 로컬 자정 기준(`today()`), 지연·기한 초과 판정도 같은 규칙(`inspectionOverdue`/`woOverdue`).

### 용어 사전 (2026-09-03)

같은 개념이 화면마다 다른 말로 불리던 것을 하나로 고정한다. 새 문구는 이 표를 따른다.

| 개념 | 확정 용어 | 쓰지 않는 표기 |
|---|---|---|
| 세 화면 이름·이동 링크 | **3D 뷰어(짧게 3D) · 모니터링 · 시설관리** — 순서도 이 순서 | 카드(시설관리 링크의 옛 이름) |
| 자산 상태 OUT_OF_SERVICE | **사용 중지** | 중지, 고장/중지 |
| 점검 기한 넘김 | **지연** (점검·자산 쪽 전용) | 초과 |
| 작업지시 기한 넘김 | **기한 초과** (짧게 초과) | 지연 |
| 미등록 요소 일괄 자산화 | **자산 일괄 등록** | 미등록 N개 자산 등록 |
| 점검 결과 | **정상 / 결함** (저장값 OK/DEFECT 은 불변) | OK, DEFECT 원문 노출 |
| 팀 표기 | 개념어는 **분야**(분야 현황·층 × 분야), 이름은 접미사 없이: 소방 · 수송 · 설비 · 통신·제어 · 전기 (칩·배지는 약칭 '통신') | 소방팀, 전기팀, 팀 현황 |
| 운영 상태 | `status.ts` 라벨만 (경보·장애·…) — 코드 원문 노출 금지 | ALARM, FAULT 원문 |
| 작업지시 '대기(OPEN)' | 중립색 배지 — 빨강은 경보·기한 초과의 몫 | crit 빨강 배지 |
...
- ✅ **점검 지연의 모니터링 노출** — 총계 줄·팀 카드 건수, "조치 필요" 최하위 순위, 행 "점검 지연" 뱃지.
