...
| 통합 범위 | 지도 위 건물 배치 + 자산/점검/작업지시 + 설비 계통(그래프 추적·운영 상태·모니터링) | — |
| 계통 그래프 | IFC 연결을 `connection` 테이블로, 추적은 PostgreSQL 재귀 CTE (그래프 DB 없음) | [02-data-model](02-data-model.md) |
| 기준 표준 | IFC 4.3 (ISO 16739-1:2024), 입력은 IFC2x3/4 허용. IDS 1.0, BCF 3.0, COBie 개념 반영 | — |

## 범위 밖 (명시)

- 사용자 인증/권한 (데모 단일 사용자). 외부 공개 시 프록시 레벨로 — [05](05-scale-and-security.md)
...
4. 요소 → 자산 등록 → 점검 → 작업지시 → 뷰어에서 하이라이트/카메라 이동. 작업지시 보드(칸반)에서 상태 이동
5. 계통 탭 → 계통별 색 → 요소 상류/하류 추적 → 상태판에서 경보 발생(BMS 시뮬레이터) → 수신기 집계·작업지시 자동 → 모니터링 격자에서 경보 클릭 → 포커스 뷰(구역 강조·비콘) → 정전 시나리오
6. (M5) IDS 검증 → 실패 요소 색상 표시
